...
public class AuthServiceImpl implements AuthService {
    ...
    public SignInKeycloakResponse register(UserRegistrationRequest request) throws AuthenticationException, IOException {
        try {
            ...
            SignInKeycloakResponse response = jwtAuthProvider.registerUser(request, createdUser.getId());
            log.info("User '{}' successfully registered in KeyCloak", request.getEmail());
            log.info("User '{}' successfully created in local database", request.getEmail());
            
            return response;
        } catch (IOException e) {
            log.error("IO error during user registration for '{}': {}", request.getEmail(), e.getMessage());
